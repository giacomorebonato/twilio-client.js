...
PeerConnection.prototype._setupChannel = function() {
  ...
  pc.onconnectionstatechange = () => {
    if (pc.connectionState === 'connected') {
      console.log(`[NOICELITE] Gathering to Connected: ${Date.now() - startTime}`);
    }
  ...
  pc.onicegatheringstatechange = () => {
    ...
    this.log(`pc.iceGatheringState is "${pc.iceGatheringState}"`);
  ...
  this.version.createOffer(this.options.maxAverageBitrate, this.codecPreferences, { iceRestart: true }).then(() => {
    ...
    this._onAnswerOrRinging = payload => {
      ...
      if (this.status !== 'closed') {
        this.version.processAnswer(this.codecPreferences, payload.sdp, null, err => {
          const message = err && err.message ? err.message : err;
          this.log(`Failed to process answer during ICE Restart. Error: ${message}`);
        });
      }
    };

    this._onHangup = () => {
      this.log('Received hangup during ICE Restart');
      this._removeReconnectionListeners();
    };
    ...
    const message = err && err.message ? err.message : err;
    this.log(`Failed to createOffer during ICE Restart. Error: ${message}`);
